...
if [ "$1" == "--help" ]; then
    ...
    echo "where:"
    echo " "
    echo "  target_board: aruba2 | cardhu | kai | dalmore | roth | bonaire(_sim) (default is aruba2)"
...
if [ "$target_board" == "dalmore" ]; then
    board_has_wifi="true"
fi

if [ "$target_board" == "roth" ]; then
    board_has_wifi="true"
fi
...
if [ "$build_flavor" == "debug" ]; then
    if [ "$target_board" == "ardbeg_t114" ]; then
        product_dir=$top/out/debug/target/product/ardbeg
    else
        product_dir=$top/out/debug/target/product/$target_board
    fi
elif [ "$build_flavor" == "release" ]; then
    if [ "$target_board" == "ardbeg_t114" ]; then
        product_dir=$top/out/target/product/ardbeg
    else
...
cp $product_dir/system/bin/nvmap_benchmark .
cp $product_dir/system/bin/nvmap_sanity .
cp $product_dir/system/bin/nvmap_sanity_child .
cp $product_dir/system/bin/nvmap_stress .
cp $product_dir/system/bin/nvtest .
cp $product_dir/system/bin/nvogtest .
cp $product_dir/system/bin/nvogtestgl .
cp $product_dir/system/bin/nvhost_* .
cp $product_dir/system/bin/framebuffer_test .
...
echo "Adding nvogtestgl data files to $product_dir/nvtest_ramdisk_tmpdir/data/graphics/nvogtestgl"
mkdir -p $product_dir/nvtest_ramdisk_tmpdir/data/graphics/nvogtestgl
cd $product_dir/nvtest_ramdisk_tmpdir/data/graphics/nvogtestgl
cp $product_dir/data/graphics/nvogtestgl/* .
cd $product_dir/nvtest_ramdisk_tmpdir/data/graphics
cp $product_dir/data/graphics/nvogcrc.txt .
...
if [ "$firmware" == "tegra12x" ]; then
    mkdir -p $product_dir/nvtest_ramdisk_tmpdir/system/etc/firmware/tegra12x
    pushd $product_dir/nvtest_ramdisk_tmpdir/system/etc/firmware/tegra12x
    cp $product_dir/system/etc/firmware/tegra12x/* .
    popd
fi
...
cp $product_dir/system/lib/libstlport.so .
...
cp $product_dir/system/lib/libcplconnectclient.so .
...
cp $product_dir/system/lib/libnvcpl.so .
cp $product_dir/system/lib/libnvcms.so .
cp $product_dir/system/lib/libglcore.so .
cp $product_dir/system/lib/libnvglsi.so .
cp $product_dir/system/lib/libEGL.so .
cp $product_dir/system/lib/libGLES_trace.so .
...
cp $product_dir/system/lib/nvddk2d_rendering_tests.so .
cp $product_dir/system/lib/libardrv_dynamic.so .
cp $product_dir/system/lib/libnvddk_vic.so .
...
cp $product_dir/system/lib/libnvrmapi_tegra.so .
...
cp $product_dir/system/lib/libtsec_wrapper.so .
...
cp $product_dir/system/lib/gles2_simplest_lines.so .
cp $product_dir/system/lib/gles2_simplest.so .
...
cp $product_dir/system/lib/egl/libGLES_android.so .
cp $product_dir/system/lib/egl/libEGL_tegra.so .
cp $product_dir/system/lib/egl/libEGL_tegra_impl.so .
cp $product_dir/system/lib/egl/libGLESv2_tegra.so .
cp $product_dir/system/lib/egl/libGLESv2_tegra_impl.so .
cp $product_dir/system/lib/egl/libGLESv2_perfhud.so .
cp $product_dir/system/lib/egl/libGLESv1_CM_tegra_impl.so .
cp $product_dir/system/lib/egl/libGLESv1_CM_tegra.so .

echo "Adding files to $product_dir/nvtest_ramdisk_tmpdir/system/lib/hw"
mkdir -p $product_dir/nvtest_ramdisk_tmpdir/system/lib/hw
cd $product_dir/nvtest_ramdisk_tmpdir/system/lib/hw
cp $product_dir/system/lib/hw/gralloc.tegra.so .
cp $product_dir/system/lib/hw/hwcomposer.tegra.so .
...
if [ -d $android_host_bin ]; then
	...
	$android_host_bin/mkbootfs $product_dir/nvtest_ramdisk_tmpdir > $product_dir/nvtest_ramdisk.img
else
