...
#define LUX_SYSFS_PATH_K38      "/sys/bus/iio/devices/iio:device0/in_illuminance_raw"
...
#define PROX_SYSFS_PATH_K38     "/sys/bus/iio/devices/iio:device0/in_proximity_raw"
...
#define PROX_THRESHOLD_ISL29028 100
...
sensors_poll_context_t::sensors_poll_context_t()
{
    ...
    if (fd < 0) {
        fd = open(LUX_SYSFS_PATH_NEW, O_RDONLY);
        if (fd < 0) {
            mSensors[light] = new Isl29028Light(LUX_SYSFS_PATH_K38, "", ID_L);
        } else {
            close(fd);
    ...
    if (fd < 0) {
        fd = open(PROX_SYSFS_PATH_NEW, O_RDONLY);
        if (fd < 0) {
            mSensors[proximity] = new Isl29028Prox(PROX_SYSFS_PATH_K38, "", ID_P,
                                        PROX_THRESHOLD_ISL29028);
        } else {
            close(fd);
            mSensors[proximity] = new Isl29028Prox(PROX_SYSFS_PATH_NEW,
                                        PROX_ENABLE_SYSFS_PATH_NEW,
