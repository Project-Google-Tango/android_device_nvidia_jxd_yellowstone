...
int MPLSensor::rvHandler(sensors_event_t* s)
{
    ...
    ALOGV_IF(HANDLER_DATA, "HAL:rv data: %+f %+f %+f %+f - %+lld - %d",
...
int MPLSensor::setLpaDelay(unsigned long us)
{
    return lpa_delay_enable(us);
}
...
int MPLSensor::readAccelEvents(sensors_event_t* data, int count)
{
   ...
   ssize_t n = mAccelInputReader.fill(mpu_int_fd);
    if (n < 0) {
...
int MPLSensor::readEvents(sensors_event_t *data, int count)
{
    ...
    if (count < 1)
        return -EINVAL;
	numEventReceived = readAccelEvents(data,count);
	return numEventReceived;
...
int MPLSensor::inv_read_temperature(long long *data)
{
    ...
    if (!strncmp(chip_ID, "MPU3050", strlen("MPU3050")))
        room_temperature = TEMP_MPU3050_ROOM_TEMPERATURE;
    else if (!strncmp(chip_ID, "MPU6500", strlen("MPU6500")))
        room_temperature = TEMP_MPU6500_ROOM_TEMPERATURE;
    else
        room_temperature = TEMP_MPU6050_ROOM_TEMPERATURE;

    data[0] = (room_temperature + ((raw - mTempOffset) / mTempScale)) * 65536.f;
    data[1] = timestamp;
...
int MPLSensor::isCompassDisabled()
{
    if (mCompassSensor == NULL)
        return 1;
