...
struct sensor_t sSensorList[8] = { // 8 is 1 bigger that the size it looks to be here
      MPLROTATIONVECTOR_DEF,       // in order to accomodate the light sensor
      MPLLINEARACCEL_DEF,
      MPLGRAVITY_DEF,
      MPLGYRO_DEF,
      MPLACCEL_DEF,
...
{
    *list = sSensorList;
    int size = ARRAY_SIZE(sSensorList) - 1;
    LightSensorBase::fillSensorDef(sSensorList, size);
    ALOGE_IF(size == ARRAY_SIZE(sSensorList) - 1, "No ALS sensor found\n");
    return size;
}
